...
object BoardFactory:
  ...
  /** Creates a board where each player's field is pre-populated with 4 cards drawn from a
   * shuffled deck.
   *
   * @return
   ...
   *
   * The `players` list must contain exactly one [[Field]] per player, in the same order as
   * [[Player.values]].
   *
   * @param players
